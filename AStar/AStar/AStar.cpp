...
uniform_int_distribution<mt19937::result_type> runif_SIZE(0, SIZEX - 1);
...
vector<Coord*> get_adj_Coords(const Coord * const n) {
	...
	if (north >= 0 && (M[north][col].type < 2)) {
		adj_Coords.push_back(&M[north][col]);
	}
	if (south <= SIZEY - 1 && (M[south][col].type < 2)) {
		adj_Coords.push_back(&M[south][col]);
	}
	if (west >= 0 && (M[row][west].type < 2)) {
		adj_Coords.push_back(&M[row][west]);
	}
	if (east <= SIZEX - 1 && (M[row][east].type < 2)) {
		adj_Coords.push_back(&M[row][east]);
	}
